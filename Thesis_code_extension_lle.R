...
# with LLE as its dimension reduction technique
...
set.seed(100)
optimal_cluster_count <- NbClust(data = lle_jester_dataset_full_subset_coordinates, diss = NULL, distance = "euclidean", min.nc = 2, max.nc = 10, method = "kmeans")
...
cluster_agnes_lle <- cutree(agnes_lle, k=4)
...
cl1 <- pam(cluster_assignments_kmeans_lle, 4) # Cluster amount
...
# (iv) Davies-Bouldin Index
cl2 <- pam(cluster_agnes_lle, 4) # Cluster amount
