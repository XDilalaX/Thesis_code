...
# of the thesis for k-means and AGNES clustering with t-SNE as its dimension 
# reduction technique
...
cluster_agnes_tsne <- cutree(agnes_tsne, k=3)
print(cluster_agnes_tsne)

# Get kmeans with t-SNE visualization
plot(tSNE_jester_dataset_full_subset_coordinates, col =cluster_assignments_kmeans_tsne, main = "K-means - with t-SNE", xlab = "V1", ylab = "V2", pch = 15)

# Get AGNES with t-SNE visualisation 
plot(tSNE_jester_dataset_full_subset_coordinates, col = cluster_agnes_tsne, main = "AGNES - with t-SNE", xlab = "V1", ylab = "V2", pch = 15)
...
# Get AGNES with t-SNE visualization
plot(tSNE_jester_dataset_full_subset_coordinates_2, col =cluster_agnes_tsne_2, main = "AGNES - with t-SNE with perplexity = 6", xlab = "V1", ylab = "V2", pch = 15)
...
cl1 <- pam(cluster_assignments_kmeans_tsne, 3)
...
# (iv) Davies-Bouldin index
cl1_1 <- pam(cluster_assignments_kmeans_tsne_1, 3)
...
# (iv) Davies-Bouldin index
cl2_1 <- pam(cluster_agnes_tsne_1, 3)
...
# (iv) Davies-Bouldin index
cl1_2 <- pam(cluster_assignments_kmeans_tsne_2, 3)
...
# (iv) Davies-Bouldin index
cl2_2 <- pam(cluster_agnes_tsne_2, 3)
...
# (iv) Davies-Bouldin index
cl1_3 <- pam(cluster_assignments_kmeans_tsne_3, 3)
...
# (iv) Davies-Bouldin index
cl2_3 <- pam(cluster_agnes_tsne_3, 3)
