...
# of the thesis for k-means and AGNES clustering with PCA as its dimension 
# reduction technique
...
# (2) AGNES with k = 2 (found in step 1)
# (ii) AGNES with PCA
agnes_pca <- agnes(pca_principal_components, metric = "euclidian", method = "average")

# Get and view the cluster assignments 
cluster_agnes_pca <- cutree(agnes_pca, k=3)
print(cluster_agnes_pca)

# Get kmeans with PCA visualization
plot(pca_principal_components, col = cluster_assignments_kmeans_pca, main = "K-means - with PCA", xlab = "PC1", ylab = "PC2", pch = 15)

# Get AGNES with PCA visualisation 
plot(pca_principal_components, col = cluster_agnes_pca, main = "AGNES - with PCA", xlab = "PC1", ylab = "PC2", pch = 15)
...
cl1 <- pam(cluster_assignments_kmeans_pca, 3) # Cluster amount
