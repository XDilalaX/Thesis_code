...
# up until 4 for the research of the thesis for k-means and AGNES clustering with ICA as its dimension 
# reduction technique
...
nb_k <- NbClust(data = ica_independent_components, diss = NULL, distance = "euclidean", min.nc = 2, max.nc = 10, method = "kmeans")
...
cluster_agnes_ica <- cutree(agnes_ica, k=2)
...
cl1 <- pam(cluster_assignments_kmeans_ica, 2) #cluster count
...
# (iv) Davies-Bouldin Index
cl2 <- pam(cluster_agnes_ica, 2) #cluster count
